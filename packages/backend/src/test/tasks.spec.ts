...
describe('Task', () => {
    ...
    after(async () => {
      ...
      if (projectId) {
        const response = await request(app)
          .delete('/project')
          .set('Authorization', `${token}`)
          .send({ organization: "OrganizationExample_2", project: "ProjectForTaskPost" });
        expect(response.status).to.equal(200);
      }
    });
  });
    ...
    after(async () => {
      ...
      if (projectId) {
        const response = await request(app)
          .delete('/project')
          .set('Authorization', `${token}`)
          .send({ organization: "OrganizationExample_2", project: "ProjectWithSprint" });
        expect(response.status).to.equal(200);
      }
    });
    ...
    after(async () => {
      ...
      if (projectId) {
        const response = await request(app)
          .delete('/project')
          .set('Authorization', `${token}`)
          .send({ organization: "OrganizationExample_2", project: "ProjectWithTasks" });
        expect(response.status).to.equal(200);
      }
    });
    ...
    after(async () => {
      ...
      if (projectId) {
        const response = await request(app)
          .delete('/project')
          .set('Authorization', `${token}`)
          .send({ organization: "OrganizationExample_2", project: "ProjectWithIncompleteTasks" });
        expect(response.status).to.equal(200);
      }
    });
    ...
    after(async () => {
      ...
      if (projectId) {
        const response = await request(app)
          .delete('/project')
          .set('Authorization', `${token}`)
          .send({ organization: "OrganizationExample_2", project: "ProjectWithAllTasks" });
        expect(response.status).to.equal(200);
      }
    });
  ...
  describe('PUT /task/update/:id', () => {
    ...
    after(async () => {
      ...
      if (projectId) {
        const response = await request(app)
          .delete('/project')
          .set('Authorization', `${token}`)
          .send({ organization: "OrganizationExample_2", project: "ProjectForTaskUpdate" });
        expect(response.status).to.equal(200);
      }
    });
    ...
    it('should update a task successfully', async () => {
      ...
      expect(response.body.error).to.equal(false);
      expect(response.body.result.description).to.equal(updatedDescription);
      expect(new Date(response.body.result.endDate).toISOString()).to.equal(updatedEndDate);
      expect(response.body.result.priority).to.equal(updatedPriority);
